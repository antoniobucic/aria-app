...
// Kein API Key mehr hier – wird sicher über Netlify Function abgerufen!
...
export default function App() {
  ...
  useEffect(() => { localStorage.setItem("aria-appointments", JSON.stringify(appointments)); }, [appointments]);
    ...
    appointments.forEach(appt => {
      ...
      if (diffMs >= 0 && diffMs <= 30000 && !notifiedIds.has(notifKey)) {
        ...
        new Notification(`${CATEGORY_ICONS[appt.category] || "📌"} ${appt.title}`, {
          body: `${reminderLabel(appt.reminderMinutes)} · ${appt.time} Uhr`,
        });
      }
    });
  ...
  async function requestNotifications() {
    ...
    setNotifStatus(perm);
    if (perm === "granted") {
      new Notification("🗓 ARIA aktiviert!", { body: "Du erhältst ab jetzt Erinnerungen." });
      setMessages(prev => [...prev, { role: "assistant", text: "✅ Benachrichtigungen aktiviert!" }]);
    }
  }
  ...
  async function sendMessage() {
    ...
    setLoading(true);
    try {
      // Ruft unsere sichere Netlify Function auf – kein API Key im Client!
  ...
  return (
    <div style={{ fontFamily: "'IBM Plex Sans', 'Segoe UI', sans-serif", background: "#0a0a0f", height: "100dvh", color: "#e2e8f0", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ padding: "env(safe-area-inset-top, 12px) 20px 0", background: "#0a0a0f", borderBottom: "1px solid #1a1a2e", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, paddingTop: 8 }}>
          ...
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            ...
            <button onClick={notifStatus === "granted" ? undefined : requestNotifications} style={{ padding: "3px 9px", background: notifCfg.bg, border: `1px solid ${notifCfg.border}`, borderRadius: 20, fontSize: 10, color: notifCfg.color, cursor: "pointer", fontWeight: 600 }}>
              {notifCfg.label}
            </button>
          </div>
        </div>
        ...
        {activeTab === "termine" && (
          <div style={{ flex: 1, overflowY: "auto", padding: 16, paddingBottom: "max(16px, env(safe-area-inset-bottom))" }}>
            ...
            {sortedAppointments.map(appt => {
              ...
              return (
                <div key={appt.id} className="appt-card" style={{ background: "#13131f", border: "1px solid #1e1e30", borderLeft: `3px solid ${prio.color}`, borderRadius: 12, padding: "14px 16px", marginBottom: 10 }}>
      ...
      {showModal && <Modal editData={editAppt} onClose={() => { setShowModal(false); setEditAppt(null); }} onSave={handleSave} />}
    </div>
  );
}
